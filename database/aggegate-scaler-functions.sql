...
SELECT COUNT(employeeNumber) as total, officeCode
FROM employees
GROUP BY officeCode
ORDER BY total DESC ;
...
SELECT state, ROUND(avg(creditLimit),2) as 'Average Credit Limit' FROM customers GROUP BY state HAVING state='NV';
...
INSERT INTO employee_new(emp_name, address, department, salary,contact_numbers)
VALUES ('Dinesh', 'Colombo', 'HR', 80000,'071-123456'),
       ('Dinesh', 'Gampaha', 'IT', 60000),
       ('Dinesh', 'Kandy', 'FIN', 90000),
       ('Dinesh', 'Galle', 'HR', 70000),
       ('Dinesh', 'Colombo', 'IT', 90000),
       ('Dinesh', 'Kandy', 'HR', 80000),
       ('Dinesh', 'Colombo', 'IT', 70000),
       ('Dinesh', 'Galle', 'FIN', 80000),
       ('Dinesh', 'Gampaha', 'HR', 80000),
       ('Dinesh', 'NULL', 'FIN', 70000),
       ('Dinesh', 'Galle', 'IT', 80000),
       ('Dinesh', 'NULL', 'IT', 60000),
       ('Dinesh', 'Colombo', 'HR', 80000),
       ('Dinesh', 'Galle', 'HR', 70000),
       ('Dinesh', 'Gampaha', 'IT', 90000),
       ('Dinesh', 'Kandy', 'FIN', 80000),
       ('Dinesh', 'Galle', 'HR', 60000);

SELECT address, ROUND(AVG(salary),2) as average_salary, COUNT(emp_id) as number_of_employees FROM employee_new GROUP BY address HAVING average_salary>75000;
SELECT COUNT(*) FROM employee_new;

# ALTER
# To add columns
ALTER TABLE employee_new ADD COLUMN contact_number VARCHAR(45) NOT NULL ;
ALTER TABLE employee_new ADD spouse_name VARCHAR(45) NOT NULL;

# to modify
ALTER TABLE employee_new CHANGE contact_number contact_numbers VARCHAR(45) NOT NULL ;
ALTER TABLE employee_new MODIFY contact_numbers VARCHAR(20) NOT NULL ;
ALTER TABLE employee_new ALTER spouse_name SET DEFAULT 'No';

SELECT * FROM employee_new;

# Sub-queries

SELECT * FROM employee_new WHERE salary< (SELECT AVG(salary) FROM employee_new) ORDER BY emp_id ASC ;
...
FROM employee_new
WHERE salary in (SELECT salary FROM employee_new WHERE salary > 75000);
...
SELECT bonus_status(60000);

SELECT emp_id,emp_name,salary, bonus_status(salary) FROM employee_new;
